<template>
  <UDashboardGroup>
    <!-- Left sidebar -->
    <UDashboardSidebar collapsible class="bg-default border-default border-r">
      ...
      <template #default="{ collapsed }">
        ...
        <ClientOnly>
          ...
          />
          <div v-else-if="status === 'pending'" class="space-y-2 p-4">
            <USkeleton v-for="i in 5" :key="i" class="h-8 w-full" />
          </div>
          <div v-else-if="status === 'error'" class="p-4 text-sm text-red-500">
            Error loading navigation
          </div>
          <div v-else class="p-4 text-sm text-neutral-500">No navigation items</div>
        </ClientOnly>
      </template>
    </UDashboardSidebar>
      ...
      <UDashboardNavbar>
        <template #left>
          <div class="flex items-center gap-2">
            <UButton to="/" variant="ghost" size="sm"> Home </UButton>
            <UButton to="/blog" variant="ghost" size="sm"> Blog </UButton>
            <UButton to="/docs" variant="ghost" color="primary" size="sm"> Docs </UButton>
          </div>
        </template>
